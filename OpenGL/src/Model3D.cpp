...
Mesh Model3D::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	if (mesh->mMaterialIndex >= 0)
	{
		...
		std::vector<MatTexture> diffuseMaps = loadMaterialTextures(material,
			aiTextureType_DIFFUSE, "texture_diffuse");
		textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
		std::vector<MatTexture> specularMaps = loadMaterialTextures(material,
...
std::vector<MatTexture> Model3D::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	std::vector<MatTexture> textures;
